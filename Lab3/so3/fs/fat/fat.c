...
int fat_open(int fd, const char *path)
{
	...
	if (ptrent->tentry == TYPE_FILE) {
	    ...
	    vfs_set_type(fd, ptrent->entry.file.dir_ptr[11] & AM_SYM ? VFS_TYPE_SYMLINK : VFS_TYPE_FILE);
	}
#endif
...
int fat_stat(const char *path, struct stat *st)
{
	...
	/* fattrib has only 1 Byte, the RWX attributes are not officially
	 * supported for FAT does not understand permissions, all files are
	 * RWX, to simulate this behavior, the permissions are encoded here
	 * with 07 (octal)
	 */
	st->st_mode = (07 << 8) + finfo.fattrib;
...
bool is_type_symlink(mode_t mode) {
	return (mode & AM_SYM);
}
...
#ifdef CONFIG_FAT_WITH_SYMLINK
	.is_type_symlink = is_type_symlink
#endif
};
