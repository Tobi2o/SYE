...
int do_open(const char *filename, int flags)
{
	...
	/* When the input file is a SYMLINK but no when if it's just created */
	if (open_fds[gfd]->type == VFS_TYPE_SYMLINK && !(flags & O_SYMLINK)) {

		/* to open the content of a symbolic link, O_NOFOLLOW and
		 * O_PATH must be set */
		if (flags & O_NOFOLLOW) {
			if (flags & O_PATH)
				return fd;
		...
		/* get size of linked filename without \0 */
		size = open_fds[gfd]->fops->lseek(fd, 0, SEEK_END);

		/* set file pointer again at the beginning */
		open_fds[gfd]->fops->lseek(fd, 0, SEEK_DATA);

		/* allocate the space to host the filename (\0 included) */
		f_link = malloc(size);
		BUG_ON(!f_link);

		/* get linked filename */
		open_fds[gfd]->fops->read(fd, f_link, size);

		fd2 = do_open(f_link, flags);
		BUG_ON(fd2 < 0);

		free(f_link);

		/* close symbolic link file */
		do_close(fd);

		/* set linked file as current open file */
		fd = fd2;
	}
#endif
...
int do_link(int fd, const char *filename) {
#ifdef CONFIG_FAT_WITH_SYMLINK
	...
	/* Creates symlink file into the filesystem
	 * FIXME Could I get a race condition ? */
	fd_link = do_open(filename, O_CREAT | O_SYMLINK | O_RDWR);

	mutex_lock(&vfs_lock);

	/* Get the global file descriptor */
	gfd_orig = vfs_get_gfd(fd);
	gfd_linked = vfs_get_gfd(fd_link);

	/* Store linked filename into the file */
	do_write(fd_link, open_fds[gfd_orig]->filename,
		strlen(open_fds[gfd_orig]->filename)+1);
...
#ifdef CONFIG_FAT_WITH_SYMLINK
	if (registered_fs_ops[0]->is_type_symlink(st->st_mode)) {

			__fd = do_open(path, O_NOFOLLOW|O_PATH);

			/* get size of linked filename without \0 */
			size = registered_fs_ops[0]->lseek(__fd, 0, SEEK_END);

			/* set file pointer again at the beginning */
			registered_fs_ops[0]->lseek(__fd, 0, SEEK_DATA);
			...
			/* get linked filename */
			registered_fs_ops[0]->read(__fd, f_link, size);
